...
import { Event, EventFormFields } from '@/types/events';
...
interface Props {
  ...
  onUpdate: (id: string, data: Partial<EventFormFields>) => Promise<void>;
  onDelete: () => Promise<void>;
}

export function EventDetailContent({ event, fieldErrors, onUpdate, onDelete }: Props) {
  const router = useRouter();
  const [editMode, setEditMode] = useState(false);
  const [confirmOpen, setConfirmOpen] = useState({ type: '', open: false });
  const [formState, setFormState] = useState<Partial<EventFormFields>>({
    title: event.title,
